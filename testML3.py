...
epoch = 1500
neurons2 = 10
lstm_model2 = fit_lstm(df_train, batchSize2, epoch, neurons2)
# forecast entire training dataset
lstm_model2.predict(train_reshaped, batch_size=batchSize2)

# create LSTM model
batchSize3 = 1
epoch = 1500
neurons3 = 100
lstm_model3 = fit_lstm(df_train, batchSize3, epoch, neurons3)
# forecast entire training dataset
lstm_model3.predict(train_reshaped, batch_size=batchSize3)     
...
rmse2 = sqrt(mean_squared_error(data[-len(predictions2):], predictions2))
rmse3 = sqrt(mean_squared_error(data[-len(predictions3):], predictions3))
...
tot_predict2 = fillData(data,predictions2)
tot_predict3 = fillData(data,predictions3)
...
plt.plot(predictions2,label='Neurons = %d, RMSE: %.3f' % (neurons2, rmse2))
plt.plot(predictions3,label='Neurons = %d, RMSE: %.3f' % (neurons3, rmse3))
plt.legend(loc='lower right')
plt.title('Neurons')
# plt.figtext(0.2,0.15,'Test RMSE: %.3f' % rmse)
plt.show()
